...
- `drag` no longer works directly on a component's `x` and `y` transform. This means any code that expects `x`/`y` motion values to update as a result of a drag gesture will no longer work. In a future version we'll reincorporate events and special motion values that will fix this.
