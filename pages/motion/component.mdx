...
Any component can be turned into a `motion` component with the `motion.custom` function.

Note that the provided component must pass `ref` to the underlying DOM element that is to be animated. `motion.custom` must not be called inside a React render function.
